...
extension OWNetworkHTTPHeader {
    ...
    ///
    /// OWNetwork offers a default Accept-Language header that accumulates and encodes the system's preferred languages.
    ...
    ///
    /// OWNetwork offers a default accept encoding value that provides the most common values. Use
    ...
    ///
    /// OWNetwork provides built-in methods to produce `Authorization` headers. For a Basic `Authorization` header use
    ...
    ///
    /// All OWNetwork `ParameterEncoding`s and `ParameterEncoder`s set the `Content-Type` of the request, so it may not be necessary to manually
    /// set this value.
    ///
...
extension OWNetworkHTTPHeader {
    /// Returns OWNetwork's default `Accept-Encoding` header, appropriate for the encodings supported by particular OS
    /// versions.
    ///
    ...
    /// Returns OWNetwork's default `Accept-Language` header, generated by querying `Locale` for the user's
    /// `preferredLanguages`.
    ///
    ...
    /// Returns OWNetwork's default `User-Agent` header.
    ///
    /// See the [User-Agent header documentation](https://tools.ietf.org/html/rfc7231#section-5.5.3).
    ///
    /// Example: `iOS Example/1.0 (org.OpenWebSDK.iOS-Example; build:1; iOS 13.0.0) OpenWebSDKNetwork/5.0.0`
    static let defaultUserAgent: OWNetworkHTTPHeader = {
        ...
        let sdkVersion = OWSettingsWrapper.sdkVersion() ?? "na"
        let openWebVersion = "OpenWebSDKNetwork/\(sdkVersion)"

        let userAgent = "\(executable)/\(appVersion) (\(bundle); build:\(appBuild); \(osNameVersion)) \(openWebVersion)"
