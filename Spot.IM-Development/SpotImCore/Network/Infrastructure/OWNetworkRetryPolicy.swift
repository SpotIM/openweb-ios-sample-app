...
class OWNetworkRetryPolicy: OWNetworkRequestInterceptor {
    ...
    static let defaultRetryableURLErrorCodes: Set<URLError.Code> = [// [Security] App Transport Security disallowed a connection because there is no secure network connection.
        ...
        // [File System] A download task was unable to the downloaded file on disk.
        //   - [Disabled] File system error is unlikely to recover with retry.
        // .cannotOpenFile,
    ...
    let retryableHTTPMethods: Set<OWNetworkHTTPMethod>
...
extension OWNetworkRequestInterceptor where Self == OWNetworkRetryPolicy {
...
extension OWNetworkRequestInterceptor where Self == OWNetworkConnectionLostRetryPolicy {
