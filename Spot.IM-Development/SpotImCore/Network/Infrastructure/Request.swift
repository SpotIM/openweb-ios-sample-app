...
class Request {
    ...
    /// `State` of the `Request`.
    var state: State { $mutableState.state }
    /// Returns whether `state` is `.initialized`.
    var isInitialized: Bool { state == .initialized }
    /// Returns whether `state is `.resumed`.
    var isResumed: Bool { state == .resumed }
    ...
    var requests: [URLRequest] { $mutableState.requests }
    ...
    var request: URLRequest? { lastRequest }
...
class DownloadRequest: Request {
    ...
    struct Options: OptionSet {
        ...
        static let createIntermediateDirectories = Options(rawValue: 1 << 0)
    ...
    var fileURL: URL? { $mutableDownloadState.fileURL }
