...
final class RevocationTrustEvaluator: ServerTrustEvaluating {
    ...
    struct Options: OptionSet {
        ...
        static let crl = Options(rawValue: kSecRevocationCRLMethod)
        /// Consult only locally cached replies; do not use network access.
        static let networkAccessDisabled = Options(rawValue: kSecRevocationNetworkAccessDisabled)
        /// Perform revocation checking using OCSP (Online Certificate Status Protocol).
        static let ocsp = Options(rawValue: kSecRevocationOCSPMethod)
        /// Prefer CRL revocation checking over OCSP; by default, OCSP is preferred.
        static let preferCRL = Options(rawValue: kSecRevocationPreferCRL)
        ...
        static let requirePositiveResponse = Options(rawValue: kSecRevocationRequirePositiveResponse)
        ...
        static let any = Options(rawValue: kSecRevocationUseAnyAvailableMethod)
    ...
    ///                               default.
    init(performDefaultValidation: Bool = true, validateHost: Bool = true, options: Options = .any) {
...
/// Uses the pinned keys to validate the server trust. The server trust is considered valid if one of the pinned
/// keys match one of the server certificate keys. By validating both the certificate chain and host,
...
final class PublicKeysTrustEvaluator: ServerTrustEvaluating {
    ...
    ///
    /// - Parameters:
    ///   - keys:                     The `SecKey`s to use to validate keys. Defaults to the keys of all
