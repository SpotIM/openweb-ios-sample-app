...
class OWCommentCreationFloatingKeyboardViewViewModel:
    ...
    fileprivate func setupInitialTextAndTypeFromCacheIfNeeded() {
        ...
        switch commentType {
        case .comment:
            guard let text = commentsCacheService[.comment(postId: postId)]?.text else { return }
            initialText = text
        case .replyToComment(originComment: let originComment):
            guard let originCommentId = originComment.id,
                  let text = commentsCacheService[.reply(postId: postId, commentId: originCommentId)]?.text
            else { return }
            initialText = text
        case .edit(comment: let comment):
            ...
            } else if let text = commentsCacheService[.edit(postId: postId)]?.text {
                initialText = text
            }
        }
    }
